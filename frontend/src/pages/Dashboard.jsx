  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-[#020617] overflow-hidden font-sans select-none antialiased">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Chat List Column: Visible on desktop when activeTab is chats, or hidden on mobile when a friend is selected */}
        <div className={`w-full md:w-[400px] border-r border-white/5 flex flex-col bg-[#020617] overflow-hidden ${activeTab === 'chats' ? (selectedFriend ? 'hidden md:flex' : 'flex') : 'hidden md:flex'}`}>
            {renderChatList()}
        </div>

        {/* Dynamic Main Content Column */}
        <main className={`flex-1 overflow-hidden relative bg-[#0B1120]/10 flex flex-col h-full ${activeTab === 'chats' && !selectedFriend ? 'hidden md:flex' : 'flex'}`}>
            <AnimatePresence mode="wait">
                <motion.div 
                    key={activeTab + (selectedFriend?._id || '')} 
